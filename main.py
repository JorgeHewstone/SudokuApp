...
class MenuScreen(MDScreen):
    ...
    def create_buttons(self):
        ...
        # Local imports for DPI-aware sizing
        from kivy.metrics import sp
        from kivymd.uix.card import MDCard
        from kivymd.uix.label import MDLabel
        from kivymd.uix.button import MDButton, MDButtonText

        # --- Metrics (responsive but stable) ---
        screen_width, screen_height = Window.size
        button_width = screen_width * 0.40         # fixed width for all difficulty items
        button_height = button_width / 1.8         # consistent aspect ratio
        spacing = screen_height * 0.02             # vertical spacing

        # --- Colors ---
        difficulty_colors = {
            "Easy":   (0.1, 0.7, 0.2, 1),
            "Medium": (0.2, 0.4, 1.0, 1),
            "Hard":   (230/255, 218/255, 0, 0.95),
            "God":    (0.8, 0.1, 0.1, 1),
        }
        text_colors = {
            "Easy":   (1, 1, 1, 1),
            "Medium": (1, 1, 1, 1),
            "Hard":   (0, 0, 0, 1),   # better contrast on yellow
            "God":    (1, 1, 1, 1),
        }

        # --- Group that keeps items centered and equal-sized ---
        total_group_height = 4 * button_height + 3 * spacing
        group = BoxLayout(
            orientation="vertical",
            spacing=spacing,
            size_hint=(None, None),
            size=(button_width, total_group_height),
            pos_hint={"center_x": 0.5, "center_y": 0.62},
        )
        self.layout.add_widget(group)

        # --- Helper to create a clickable card with auto-scaling text ---
        def make_difficulty_card(difficulty: str):
            # Card – fixed size, custom bg color
            card = MDCard(
                size_hint=(1, None),
            ...
            # Defensive: avoid any adaptive growth
            try:
                card.adaptive_size = False
                card.adaptive_width = False
                card.adaptive_height = False
            except Exception:
                pass
            ...
            lbl = MDLabel(
                ...
                text_color=text_colors[difficulty],
                size_hint=(1, 1),
            )

            # Keep text centered within
            card.bind(size=lambda inst, val: setattr(lbl, "text_size", val))

            # DPI-aware font scaling bound to card height
            def _sync_font_size(*_):
                lbl.font_size = sp(card.height * 0.3)  # 45% of height
            _sync_font_size()
            card.bind(height=lambda *_: _sync_font_size())

            # Tap handler
            def _on_release(*_):
                self.on_difficulty_selected(card)
        ...
        # --- Add four equal cards with responsive text ---
        for diff in ["Easy", "Medium", "Hard", "God"]:
            group.add_widget(make_difficulty_card(diff))

        # --- Advanced Options (fixed size, bottom center, responsive text) ---
        ...
        try:
            adv_btn.adaptive_size = False
            adv_btn.adaptive_width = False
            adv_btn.adaptive_height = False
        except Exception:
            pass
        adv_btn.size_hint_min = (0, 0)

        adv_lbl = MDButtonText(
            text="Advanced Options",
            theme_text_color="Custom",
            text_color=(1, 1, 1, 1),
        )
        # DPI-aware font scaling bound to button height
        def _sync_adv_font(*_):
            adv_lbl.font_size = sp(adv_btn.height * 0.4)
            adv_lbl.text_size = adv_btn.size
        _sync_adv_font()
        adv_btn.bind(size=lambda *_: _sync_adv_font())

        adv_lbl.halign = "center"
        adv_lbl.valign = "middle"
        adv_btn.add_widget(adv_lbl)
